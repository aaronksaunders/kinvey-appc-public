...
( function(undefined) {
		...
		Kinvey.Net = {
			...
			factory : function(api, collection, id) {
				...
				} else if('undefined' !== typeof exports) {// node.js
		...
		Kinvey.Net.Http = Base.extend({
			...
			_getDeviceInfo : function() {
				...
				var ua = Titanium ? Titanium.userAgent : navigator.userAgent.toLowerCase();

				var rAppC = /(Appcelerator Titanium)\/([\w]+)/;
				...
				var browser = rAppC.exec(ua) || rChrome.exec(ua) || rSafari.exec(ua) || rFirefox.exec(ua) || rOpera.exec(ua) || rIE.exec(ua) || [];
				...
				return [navigator.platform, browser[1] || navigator.appName, browser[2] || 0, 0 // always set device ID to 0.
				].map(function(value) {
			...
			_getUrl : function() {
				...
				switch(this.api) {
					case Kinvey.Net.APPDATA_API:
						...
						break;
					case Kinvey.Net.BLOB_API:
						url = this.ENDPOINT.BLOB + '/' + Kinvey.appKey + '/' + this.collection;
						if(null != this.id) {
							url += '/' + this.id;
						}
						break;
				...
				} else if(Kinvey.Net.READ === this.operation && this.api !== Kinvey.Net.BLOB_API) {
					param.push('query={}');
				}

				if(this.api !== Kinvey.Net.BLOB_API) {
			...
			_process : function(options) {
				if(Titanium) {
					...
					Ti.API.info(' ' + this._getAuth());

					request.setRequestHeader('Authorization', 'Basic ' + Titanium.Utils.base64encode(this._getAuth()));
					request.setRequestHeader('X-Kinvey-Device-Information', this._getDeviceInfo());
					...
					request.onreadystatechange = function(resp) {
						// HORRIBLE HACK TO DEAL WITH REDIRECT, NEEDS MORE THOUGHT
						var isDownloadRequest = (self._getUrl().indexOf("download-loc") !== -1)
						if(request.status === 302 && isDownloadRequest) {
							Ti.API.info(' request.getResponseHeader[Location] ' + request.getResponseHeader("Location"));
							var response = {
								URI : request.getResponseHeader("Location")
							};
							self._handleResponse("200", JSON.stringify(response), options);
							request.abort();
							return;
						}
					...
					request.onload = function() {
						// HORRIBLE HACK TO DEAL WITHG REDIRECT, NEEDS MORE THOUGHT
						var isDownloadRequest = (self._getUrl().indexOf("download-loc") !== -1)
						if(request.status === 302 && isDownloadRequest) {
							return;
						}
					...
					// Fire request.
					// needed to send the json object ?? why
